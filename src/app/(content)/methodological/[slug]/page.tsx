...
export default async function MethodologicalSection({
  searchParams,
  params: { slug },
}: {
  ...
  return (
    <>
      <div className="w-full mb-4 mt-10 gap-4 relative flex justify-between items-center pb-4 border-b-2 border-foreground">
        ...
        <div className='flex flex-col gap-3'>
            <h1 className="text-foreground lg:text-4xl sm:text-3xl text-2xl font-NotoSerif font-bold">
                Методические ресурсы: <span className='lg:text-2xl text-xl font-medium'>{dataResult.status !== "rejected" && dataResult.value.attributes.title}</span>
            </h1>
      ...
      <Suspense fallback={
        ...
        <MethodologicalContent section={slug} searchParams={searchParams} />
      </Suspense>
    </>
  )
}
